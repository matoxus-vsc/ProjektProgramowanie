...
Bot::Bot(int health, int attack) : Player(health, attack),
    ...
    hide_timer(0), patrol_timer(0), patrol_dir(1), fire_cooldown(0), goal_change_timer(0), detection_range(480.0f),
    movement_timer(0), strafe_dir_x(0.0f), strafe_dir_y(0.0f)
{
...
void Bot::updateAI(Player& enemy)
{
    ...
    if (enemyVisible)
    {
        ...
        if (!is_shooting && fire_cooldown == 0 && !is_reloading())
        {
            if (ammo_in_mag_get() > 0)
            {
                ...
                hide_timer = 35;
                fire_cooldown = 6 + std::rand() % 5;  // Randomowy cooldown
    ...
    bullet_x += bullet_dx; 
    bullet_y += bullet_dy;
    if (bullet_x < 0 || bullet_x > arena.map_width || bullet_y < 0 || bullet_y > arena.map_height) 
    { 
        is_shooting = false; 
    }
}
...
void Bot::change_goal()
{
    ...
    // Losowy punkt na mapie (całe pole, nie tylko środek)
