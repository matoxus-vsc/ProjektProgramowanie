...
bool media(Technical& t)
{
    ...
    for (auto &b : bots)
    {
        if(!b.sprite.texture_load("ProjektProgramowanie/tex/gracz.png"))
        {
            success = false;
    ...
    if(!player.gun.texture_load("ProjektProgramowanie/tex/bron.png"))
    {
        success = false;
    ...
    for (auto &b : bots)
    {
        if(!b.gun.texture_load("ProjektProgramowanie/tex/bron.png"))
        {
            success = false;
    ...
    if(!door_obj.texture_load("ProjektProgramowanie/tex/drzwi.png"))
    {
        success = false;
        SDL_Log("Couldnt load 'drzwi.png'! %s", SDL_GetError());
    }
    if(!map_obj.texture_load("ProjektProgramowanie/tex/mapa.png"))
    {
        success = false;
        SDL_Log("Couldnt load 'mapa.png'! %s", SDL_GetError());
    }
    if(!player.bullet.texture_load("ProjektProgramowanie/tex/pocisk.png"))
    {
        success = false;
    ...
    for (auto &b : bots)
    {
        if(!b.bullet.texture_load("ProjektProgramowanie/tex/pocisk.png"))
        {
            success = false;
    ...
    if(!wall_obj.texture_load("ProjektProgramowanie/tex/sciana.png"))
    {
        success = false;
